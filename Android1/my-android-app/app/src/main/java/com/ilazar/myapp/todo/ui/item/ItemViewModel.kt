...
class ItemViewModel(private val itemId: String?, private val itemRepository: ItemRepository) :
    ...
    fun saveOrUpdateItem(insulinDate: String, insulinTaken: Boolean, insulinAmount: Int, text: String) {
        viewModelScope.launch {
            Log.d(TAG, "saveOrUpdateItem...");
            try {
                uiState = uiState.copy(isSaving = true, savingError = null)
                val item = uiState.item?.copy(insulinDate = insulinDate, insulinTaken = insulinTaken, insulinAmount = insulinAmount, insulinType = text)
                if (itemId == null) {
